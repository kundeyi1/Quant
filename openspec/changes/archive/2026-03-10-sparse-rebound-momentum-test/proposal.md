...
- **数据适配**: 开发脚本支持读取中证 500 指数（Excel 格式）及中信一级行业指数文件 `D:/DATA/INDEX/ZX/ZX_YJHY.csv`（Wind 格式）。系统 MUST 使用 `core.DataManager` 提供的 Wind 格式解析能力来读取、标准化并对齐这些数据。
- **结果统计**: 计算信号触发后 T+1 至 T+20 的行业累积收益、分组单调性及胜率。
